...
IperfInterface::IperfInterface(QString initialArguments,
  ...
  this->logFile->open(QIODevice::ReadWrite | QIODevice::Truncate |
                      QIODevice::Text);
  this->logFile->close();

  this->logFileWatcher =
      new QFileSystemWatcher(QStringList(this->logPathAndFilename));
  QObject::connect(this->logFileWatcher, SIGNAL(fileChanged(QString)), this,
                   SLOT(processFileChanged(QString)));
}
...
QString IperfInterface::getInitialArguments() { return this->initialArguments; }
...
bool IperfInterface::getIsServerListening() { return this->serverIsListening; }
...
QMap<QString, QString> IperfInterface::getNetworkInterfaces() {
  ...
  try {
    foreach (const QNetworkInterface &interface,
      ...
      foreach (const QNetworkAddressEntry &interfaceAddressEntry,
               interface.addressEntries()) {
        if (!interfaceAddressEntry.ip().isNull() &&
            !interfaceAddressEntry.ip().isLoopback() &&
            interfaceAddressEntry.ip().protocol() ==
                QAbstractSocket::IPv4Protocol) {
          networkInterfaces.insert(interface.name(),
                                   interfaceAddressEntry.ip().toString());
        }
      }
    }
...
void IperfInterface::run() {
  QStringList parsedArguments =
      this->parseCombinedArgString(QString(this->initialArguments)
                                       .append(" --logfile ")
                                       .append(this->logPathAndFilename));
...
void IperfInterface::processReadyReadStandardOutput() {
  ...
  foreach (QString line, lines) { emit this->logOutput(line); }
}
...
void IperfInterface::parseLogOutput(const QString &logOutput) {
  if (!this->clientConnected &&
      logOutput.contains(QRegExp(MSG_CONNECTION_ESTABLISHED))) {
    emit this->connectionEstablished();
