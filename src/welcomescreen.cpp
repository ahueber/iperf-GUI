...
WelcomeScreen::WelcomeScreen(QWidget *parent) : QWidget(parent) {
  ...
  QObject::connect(clientButton, SIGNAL(clicked()), this,
                   SLOT(onClientButtonClicked()));
  QObject::connect(serverButton, SIGNAL(clicked()), this,
                   SLOT(onServerButtonClicked()));
...
void WelcomeScreen::onExitButtonClicked() { this->close(); }
