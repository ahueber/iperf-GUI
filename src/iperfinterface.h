 ...
 *
 * Diese Klasse fungiert als Schnittstelle zwischen der GUI-Applikation und dem
 * Kommandozeilenprogramm iperf3.
 *
 * @author Tobias Merz
 */
...
//#define MSG_CONNECTION_ESTABLISHED "[a-zA-Z0-9]+ (?:[0-9]{1,3}.){3}[0-9]{1,3}
//port [0-9]+ connected with (?:[0-9]{1,3}.){3}[0-9]{1,3} port [0-9]+"
...
#define MSG_CLIENT_UNEXPECTEDLY_CLOSED                                         \
...
public:
   ...
   * @param initialArguments Die Argumente-Liste, welche an das
   * Kommandozeilenprogramm übermittelt werden.
   * @param logPathAndFilename Der Pfad zur Logdatei.
   */
  IperfInterface(QString initialArguments = QString(),
                 QString logPathAndFilename = QString("/tmp/iperf3.log"));
  ~IperfInterface();
...
signals:
  ...
  /**
   * @brief Signal wenn die Verbindung zwischen Client und Server geschlossen
   * wurde.
   */
